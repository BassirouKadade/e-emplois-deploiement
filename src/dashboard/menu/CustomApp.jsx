...
import '../../styles/dashboard/customMenu.css'
import { useQuery } from "react-query";
import { getTotalGroupeSalleFormateurEmplois } from "../../request/createEmploisRequest/createEmploisRequest";
...
const useAnimateProgress = (maxValue, duration) => {
  const [value, setValue] = useState(0);

  useEffect(() => {
    let start = 0;
    const increment = maxValue / (duration / 100); // Calculate increment based on duration
...
export default function CustomApp() {
  ...
  const value = useAnimateProgress(dataMemo || 0, 300);

  // Memoizing the valueActuel if necessary (based on your actual use case)
      ...
      <div className="content-div1">
        <article className="article1">
          <span className="article1-span">
            ...
            {isLoading || !data? (
      <Skeleton className="skeletom" height={"40px"} baseColor="rgba(0, 167, 111,0.1)" highlightColor="rgba(0, 167, 111,0.3)" />
    ) :
    <>
            <span>{data?.user?.nom} {data?.user?.prenom}</span>
            <span style={{fontSize:"19px"}}>{data?.etablissement?.nom} </span>
            </>
  }
          </span>
         ...
         <Link to="/dashboard/creer-emploi"> <button className="articleButton">Commencer</button></Link> 
        </article>
      ...
      <div style={{padding:"8px"}} className="content-div2">
     
     
      
      {isLoading || !data? (
        <>
      <Skeleton className="skeletom spanProgresse" baseColor="#f8f8f8" highlightColor="#ffffff" />
      </>
    ) : (
       <>
      <CircularProgressbar
        ...
        styles={buildStyles({
          strokeLinecap: "butt",
          textSize:"10px",
          pathColor:getProgressColor(parseFloat(data?.etat)), // Couleur de la barre de progression
          textColor: 'gray', // Couleur du texte
        })}
      />
      <Tag style={{marginTop:"10px"}} icon={ data?.etat<100&&<SyncOutlined spin />} color="rgb(0, 167, 111,0.9) ">
      Etat d'avancement des groupes du centre
      </Tag>
      </>
    )}
...
<section className='contentBord'>
  <Link to='/dashboard/groupe/liste-groupe'>
    {isLoading || !data? (
  ...
  <Link to='/dashboard/salle/liste-salle' style={{margin:"0 10px"}}>
    {isLoading || !data ? (
      <Skeleton className="skeletom" baseColor="#f8f8f8" highlightColor="#ffffff" />
    ) : (
      <div className='lienDIv'>
  ...
  <Link to='/dashboard/formateur/liste-formateur'>
    {isLoading || !data? (
      <Skeleton className="skeletom" baseColor="#f8f8f8" highlightColor="#ffffff" />
    ) : (
      <div className='lienDIv'>
