...
export default function ListeSalle() {
  ...
  const [idCurrentSalleEmplois,setIdCurrentSalleEmplois]=useState(null)
  const options = [
    {
    ...
    {
      2: (
        <div  onClick={()=>toggleCurrentIDSelected(idCurrentSalleEmplois,"salle")}  style={{ fontSize: "16px", display: "flex", alignItems: "center" }}>
...
return (
  ...
  {contextHolder}
  <Helmet>
      <title>Liste-Salle | E-Emplois</title>        <meta name="description" content="Gérez votre profil et vos emploi du temps sur E-emplois. Accédez aux statistiques et aux informations importantes." />
        ...
        >
          ...
          dataGlobal.map((formateur, index) => (
            <tr key={index}>
              ...
              <td onMouseOver={()=>setIdCurrentSalleEmplois(formateur?.id)}>{formateur?.emplacement}
                <div className='edition'>
                  ...
                  <IconButton
                    ...
                    onClick={(event) => {
                      handleClick(event)
                      setIdCurrentSalleEmplois(formateur?.id)
