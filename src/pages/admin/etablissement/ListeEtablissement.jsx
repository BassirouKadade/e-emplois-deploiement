...
import EtablissementMod from './EtablissementMod';
...
import { obtenirListeEtablissements ,rechercherEtablissementPageSuivante,supprimerEtablissement} from '../../../request/etablissementRequest/etablissementRequest';
...
export default function ListeEtablissement() {
  ...
  const { data, isLoading } = useQuery(['liste-etablissements',currentPage], async () => {
    try {
      const response = await obtenirListeEtablissements(currentPage);
  ...
  function onChangeInput(e, index) {
   ...
   const { mutate } = useMutation(async (data) => {
     try {
       await supprimerEtablissement(data);
     } catch (error) {
       console.error(error);
     }
   }, {
   ...
   const handleOk = async () => {
     ...
     try {
       ...
       setOpenDelete(false);
       message.success("L'etablissement a été supprimé avec succès", 2);
       setDataToDelete([]);
     } catch (error) {
  ...
  const openNotification = () => {
    api.open({
      ...
      message: 'Modification Etablissement',
      description: 'Etablissement est modifié avec succès',
      icon: (
        <SmileOutlined
          style={{
            color: '#007bff',
  ...
  const { isLoading: loadingDataSearch, data: dataSearchValue } = useQuery(
    ['search-etablissements', formDataSearch],
    async () => {
      try {
        // Ajouter une petite pause pour améliorer l'expérience utilisateur
        const response = await rechercherEtablissementPageSuivante(currentPageRechercher, formDataSearch);
  ...
  const handleRemoveCache = () => {
    queryClient.removeQueries(['search-etablissements',valInit]);
  };
    ...
    switch(referenceData) {
        case "nom":
          ...
          case "adresse":
              setDataGlobal(prev => {
                  if (order === "desc") {
                      return prev.sort((a, b) => b.adresse.localeCompare(a.adresse));
                  } else {
                      return prev.sort((a, b) => a.adresse.localeCompare(b.adresse));
                  }
              });
              break;
              case "id_user":
                setDataGlobal(prev => {
                    if (order === "desc") {
                        return prev.sort((a, b) => b.id_userr - a.id_userr);
                    } else {
                        return prev.sort((a, b) => a.id_userr - b.id_userr);
                    }
                });
                break;
        default:
...
return (
    <section className='formateurs-container'>
         <Helmet>
      <title>Liste-Etablissement | E-Emplois</title>        <meta name="description" content="Gérez votre profil et vos emploi du temps sur E-emplois. Accédez aux statistiques et aux informations importantes." />
  ...
  {dataToDelete.some(element => element.delete === true) &&
    <div className='supressionData'>
      <span style={{ display: "flex", alignItems: "center" }}>
        ...
        } <span> etablissement sélectionnées</span> </span>
      </span>
    ...
    <thead>
      {/* En-têtes de colonne */}
      {
        ...
        <tr>
          ...
          <th>
            <button onMouseOver={() => setOverButtonTh("adresse")} className="buttonTH" onClick={() => handleSort('adresse')}>Adresse <FaArrowUp className={`${currentValueTri === "adresse" && order === "asc" ? "buttonTH-icons-rotate" : ""}  ${overButtonTh === "adresse" ? "iconsDiaplay" : ""} buttonTH-icons`} /> </button>
          </th>
          <th>
            <button onMouseOver={() => setOverButtonTh("id_user")} className="buttonTH" onClick={() => handleSort('id_user')}>Directeur <FaArrowUp className={`${currentValueTri === "id_user" && order === "asc" ? "buttonTH-icons-rotate" : ""}  ${overButtonTh === "id_user" ? "iconsDiaplay" : ""} buttonTH-icons`} /> </button>
          </th>
        </tr>
      }
    </thead>
  ...
  <DialogContext setOpen={setOpen} open={open}>
    <EtablissementMod openNotification={openNotification} handleClose={handleClose} currentPages={{ totalPages, currentPageRechercher, setIsSearching, currentPage, }} etablissement={moduleMod} />
  </DialogContext>
</section>
  );
}
